...
static int states_mem_accesses_eq(struct state *s1, struct state *s2)
{
    ...
    qsort(s1->mem_accesses, s1->num_mem_accesses, sizeof(struct mem_access), mem_access_cmp);
    qsort(s2->mem_accesses, s2->num_mem_accesses, sizeof(struct mem_access), mem_access_cmp);
...
static int states_eq(struct state *s1, struct state *s2)
{
    return s1->reg16.AF == s2->reg16.AF && s1->reg16.BC == s2->reg16.BC &&
           s1->reg16.DE == s2->reg16.DE && s1->reg16.HL == s2->reg16.HL && s1->PC == s2->PC &&
           s1->SP == s2->SP && s1->halted == s2->halted &&
...
static int test_instructions(size_t num_instructions, struct test_inst *insts, const char *prefix)
{
    ...
    printf("Tested %zu/%zu %sinstructions", num_instructions_tested, num_instructions, prefix);
...
static void print_coverage(bool *op_table, bool (*is_valid)(u8), const char *prefix)
{
...
int tester_run(struct tester_flags *app_flags, struct tester_operations *app_tcpu_ops)
{
    flags = app_flags;
